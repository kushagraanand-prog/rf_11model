...
@st.cache_resource
def load_artifacts():
    model = joblib.load("rf_11model.pkl")
...
try:
    ...
    st.error("Error loading artifacts. Ensure rf_11model.pkl, X_train.npy, y_train.npy, feature_order.pkl exist.")
    st.stop()
...
with st.expander("Blend Composition", expanded=True):
    ...
    with col1:
        Cu = st.number_input("Cu", value=mean_default('Cu'))
...
input_map = {
    'Cu': Cu,'Fe': Fe,'S': S, 'Al2O3': Al2O3, 'CaO': CaO, 'MgO': MgO,
...
if mode == "Counterfactual (GA)":
    ...
    if st.button("Find counterfactuals (GA)"):
        ...
        if len(succ) == 0:
            st.warning("No candidate reached the required probability.")
            best_cf = max(results, key=lambda x: x["prob"][desired_class])
        else:
            best_cf = max(succ, key=lambda x: x["prob"][desired_class])
        ...
        st.metric(
            ...
            f"{prob[desired_class]*100:.2f}%"
        )
        
        st.write(
            f"Predicted Probabilities: "
            f"{np.array2string(prob, precision=4)}"
        )
